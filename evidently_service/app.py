    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        for metric, value, labels in self.monitoring[dataset_name].metrics():
            ...
            if found is None:
                ...
                logging.info(f"logged to prometheus {metric_key}")
            ...
            return_values[metric_key] = {'value': value, "labels": labels}
...
def configure_service():
    ...
    global SERVICE
    config = {"service":{'datasets_path': 'path_to_ds.kek',
                        'min_reference_size': 30, 
                        'use_reference': True,
                        'moving_reference': False,
                        'window_size': 5,
                        'calculation_period_sec': 2},
                "datasets": {"leads": {"column_mapping": {"numerical_features": ["fico_score", "lead_sold", "dayofweek_created_at", "successful_quote_count", "age"]},
                                        "monitors": ['data_drift'],
                                        "reference_file": "project_reference_data.csv"}}}
...
def iterate(dataset: str):
    ...
    returned_metrics = SERVICE.iterate(dataset_name=dataset, new_rows=pd.DataFrame.from_dict(item))
    
    if returned_metrics is None:
        return "Noe enought data", 200
    else:
